...
useradd -M -s /bin/false snxsn
echo "snxsn:$PASS" | chpasswd
echo "snxsn" > pass.txt
echo "$PASS" >> pass.txt
...
# info
clear
echo "SN InjectoR | 081515292117" | tee log-install.txt
...
echo "---------------"  | tee -a log-install.txt
echo "User     : snxsn"  | tee -a log-install.txt
echo "Password : snxsn"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
